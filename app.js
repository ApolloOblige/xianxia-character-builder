...
function packageSummary(expanded=false) {
  ...
  if(!pack)return '<p>Choose an origin to see its starting package. A custom saved origin has no automatic allocation.</p>';
  return '<div class="starting-kit"><h3>Starting package · '+esc(pack.origin)+'</h3><p>'+esc(pack.description)+'</p><p class="purse"><strong>'+pack.spiritStones+' low-grade spirit stones</strong> · <strong>'+pack.silverTaels+' silver taels</strong></p><small>Separate currencies; no fixed exchange rate is assumed.</small><details'+(expanded?' open':'')+'><summary>View starting supplies</summary><ul>'+pack.items.map(i=>'<li><strong>'+i.quantity+' × '+esc(i.name)+'</strong>'+(i.note?'<small>'+esc(i.note)+'</small>':'')+'</li>').join('')+'</ul></details><p class="draft">Starting allocation, not a refilling inventory. These quantities are original rules for this game, informed by xianxia sources. Equipment grants no automatic stat bonuses.</p><details><summary>Design and sources</summary><p>Package version '+pack.version+' is preserved in your save. Changing origin replaces this package; your equipment notes remain separate.</p><p><a href="ORIGINS.md">Package design and full money/supply table</a></p><ul>'+R.originSources.map(x=>'<li><a href="'+x.url+'" target="_blank" rel="noopener">'+esc(x.title)+'</a></li>').join('')+'</ul></details></div>';
}
function refinedHeritage() {
  const group=character.fields.heritageCategory;
  if(!group)return '<p>Choose a broad archetype first.</p>';
  const list=R.heritageGroups[group];
  if(Array.isArray(list))return selector('heritage',list,'Choose a refined heritage')+'<small>'+list.length+' choices from your original beast wheel. Selecting a name grants no automatic powers or modifiers.</small>'+(group==='Mythic Beast'?'<p>Mythic names describe heritage, not a starting cultivation realm or guaranteed divine powers.</p>':'');
  return field('heritage',group==='Plant Spirit (custom)'?'Your original beast wheel has no plant catalogue. Enter your plant heritage here.':'Your saved or custom heritage. It is kept exactly as entered.');
}
function heritageSelector() {
  return selector('heritageCategory',R.heritageCategories,'Choose a broad archetype')+'<div id="heritage-choice">'+refinedHeritage()+'</div><small>The original wheel’s 550 unique names are preserved. Categories are browsing groups, not scientific taxonomy or rarity weights.</small>';
}
