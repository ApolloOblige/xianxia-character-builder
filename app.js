...
function review() {
  ...
  <dl class="sheet-fields">${R.fields.filter(k=>k!=='name').map(k=>`<div><dt>${labels[k]}</dt><dd>${esc(character.fields[k])||'Not set'}</dd></div>`).join('')}</dl>
  ${statTable(c)}<h3>Modifiers and conditions</h3>${modifierList()}${c.fire?'<p><strong>+1 — Fire Root</strong>: only when aggressively manipulating fire qi. Source: RPG conversation, §20 (proposal). Does not increase a core attribute.</p>':''}${R.rootNotes[character.fields.root]?`<p>${esc(R.rootNotes[character.fields.root])}</p>`:''}
...
function render(focus=false) {
  ...
  let body='';
  if(n===0) body='<p>Choose an example from a field’s suggestions, or write your own. Blank fields can be completed later.</p>'+field('name')+field('path','These four paths are preserved from your existing website.')+field('origin','Suggested origins from the RPG draft; no automatic bonuses.')+field('heritage','Your own setting choice. No heritage modifiers have been defined.');
...
document.addEventListener('input', e=>{
  const t=e.target;
  if(t.dataset.field){character.fields[t.dataset.field]=t.value; if(t.dataset.field==='root') $('root-note').textContent=R.rootNotes[t.value]||'Custom roots have no automatic effect until rules are defined.';}
